...
;;;; myam provides the ability to automatically generate a
    ...
    do (let ((result (perform-random-testing/run-once generators body)))
         (ecase (first result)
           ...
           (:fail
            ...
            (return-from perform-random-testing nil))
           (:error
            (add-result 'for-all-test-unexpectedly-failed
                        :reason "Found test data producing an error"
                        :random-state random-state
                        :failure-values (second result)
                        :result-list (third result)
                        :condition (fourth result))
            (return-from perform-random-testing nil))))
...
(defclass for-all-test-unexpectedly-failed
    (unexpected-test-failure for-all-test-failed)
  ())
...
(defmethod reason ((result for-all-test-failed))
  (format-reason-to-string "Falsifiable with ~S"
                           (slot-value result 'failure-values)))
    ...
    (loop
      ...
      until (and char
                 (or (not alphanumericp)
                     (alphanumericp char)))
      when (= 1000 count)
        do (error "After 1000 iterations ~S has still not generated ~
                  ~:[a valid~;an alphanumeric~] character :(."
                  code alphanumericp)
      finally (return char))))
...
(defun gen-member (sequence)
  (let ((length (length sequence)))
    (lambda ()
      (elt sequence (random length)))))

(defun gen-funcall (fn generator)
  "Call fn on every value that generator produces."
  (lambda ()
    (funcall fn (funcall generator))))
